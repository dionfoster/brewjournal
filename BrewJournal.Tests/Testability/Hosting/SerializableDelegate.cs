        ...
        internal SerializableDelegate(SerializationInfo info, StreamingContext context)
        {
            ...
            else
            {
                ...
                Delegate =
                    (TDelegate)
                        (object)
                            global::System.Delegate.CreateDelegate(delegateType, anonymousClassWrapper.TargetInstance,
                                methodInfo);
            }
        }
                ...
                foreach (var field in classType.GetFields())
                {
                    if (typeof(Delegate).IsAssignableFrom(field.FieldType))
                        ...
                        field.SetValue(TargetInstance,
                            ((SerializableDelegate<TDelegate>)
                                info.GetValue(field.Name, typeof(SerializableDelegate<TDelegate>))).Delegate);
